...
bool XmlStream::parseNodes ( XmlNode * pNode, XmlParser & parser, char * buffer, long parseLength )
{
	// #DGH note
	...
	{
		XmlNode * pNodesNode = new XmlNode;
		pNode->Vals.push_back(pNodesNode);

		// if the value has a tag marker
		if ( parser.hasValue() && parser.valueHasTag() )
		{
			// show found node
			...
			pNodesNode->Name = name;
			pNodesNode->bLeaf = false;
			...
			if ( !parseNodes(pNodesNode, parserNode,valueBuffer,valueLength) ) {
				parser._bError = 1;
				return false;
			}
		...
		else
		{
			...
			pNodesNode->Name = name;
			pNodesNode->bLeaf = true;
			pNodesNode->Value = value;
...
bool XmlStream::parse ( char * buffer, long length )
{
	...
	// parse nodes
	_NodeHead = new XmlNode;
	_NodeHead->Name = "Top";
	_NodeHead->bLeaf = false;
	bool docParsed = parseNodes(_NodeHead, parser,buffer,length);
	if (!docParsed) {
		delete _NodeHead;
		_NodeHead = NULL;
	}
