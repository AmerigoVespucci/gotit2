...
void CGotitEnv::InitLoadFromNLP()
{
	...
	ModNamesCntrlNLP.LoadModuleNames(	"D:\\guten\\NLPOut", cbIfExt, ".xml", !cbRemoveExt, 
	...
	while (bKeepGoing) {
		...
		ConvertStanfordOutput("D:\\guten\\GotitFilesEx", ModName, ".mod", bAddToDB,
...
void CGotitEnv::InitLoadProcessed()
{
	...
	ModNamesCntrl.LoadModuleNames("D:\\guten\\GotitFilesEx", cbIfExt, ".mod",
		!cbRemoveExt, "",
...
void CGotitEnv::MakeGroupsFromData()
{
	...
	for (uint isl = 0; isl < SentenceRec.size(); isl++) {
		...
		int TotalNumGrps = 1;
		int NumParamsPerGrp = sizeof(SrcTbl) / sizeof(*SrcTbl);
