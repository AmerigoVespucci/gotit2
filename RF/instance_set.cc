...
/***
* Named constructor for loading from a csv and label 
* vector of strings. Works same as rows of file
* Makes simpler to have a separate label file.
* @param csv_data CSV filename
* @param header whether there is a header with var. names
* @param delim CSV delimiter - defaults to ','
*/
InstanceSet* InstanceSet::LoadFromSS(stringstream& data,
										stringstream& labels,
										bool header,
										const string& delim) {
	return new InstanceSet(data, labels, header, delim);
}
...
InstanceSet::InstanceSet(stringstream& data,
            stringstream& labels,
            bool header, const string& delim) {
				//{
				//	ofstream tester("test.bin");
...
InstanceSet::InstanceSet(const string& csv_data, unsigned int* seed,
  ...
  for (int i = 0; i < original_set_size; ++i) {
    for (uint j = 0; j < attributes_.size(); ++j) {
      // uniformly sample from attributes_[j][0-original_set_size-1]
      int select = rand() % original_set_size;
...
void InstanceSet::write_csv(ostream& out, bool header,
                            const string& delim) {
  if (header) {
    for (uint i = 0; i < num_attributes() - 1; ++i) {
  ...
  for (uint i = 0; i < attributes_[0].size(); ++i) {
    for (uint j = 0; j < num_attributes() - 1; ++j) {
...
void InstanceSet::write_transposed_csv(ostream& out,
  ...
  for (uint j = 0; j < num_attributes(); ++j) {
    for (uint i = 0; i < attributes_[0].size() - 1; ++i) {
      out << attributes_[j][i] << delim;
    }
...
void InstanceSet::sort_attribute(const vector<float>& attribute,
    ...
    for (uint i = 0; i < attribute.size(); ++i) {
...
InstanceSet::InstanceSet(const InstanceSet& set,
  ...
  for (int i = 0; i < weights.size(); ++i) {
    if (weights[i] == 0) {
      // append instance
      for (uint j = 0; j < set.num_attributes(); ++j) {
...
void InstanceSet::permute(int var, unsigned int *seed) {
  ...
  for (uint i = 0; i < attr.size(); ++i) {
    int idx = rand() % labels_.size(); // randomly select an index
