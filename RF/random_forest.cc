...
{
	cout << "bad contructor for RandomForest called\n";
}
/**
...
void RandomForest::compute_skewed_proximity(const InstanceSet& set,
  ...
  for (uint i = 0; i < prox->size(); ++i) {
    for ( uint j = 0; j < prox->size(); ++j) {
...
void RandomForest::variable_importance(vector< pair< float, int> >*ranking,
  ...
  for (uint i = 0; i < trees_.size(); ++i) {
    ...
    //aggregate
    for (uint j = 0; j < tree_importance.size(); ++j) {
  ...
  for (uint i = 0; i < importances.size(); ++i) {
  ...
  // Write the z-scores
  for (uint i = 0; i < raw_scores.size(); ++i) {
