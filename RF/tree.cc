...
Tree::Tree(istream& in):
              ...
              set_(*(new InstanceSet())), // I put this in!!! Now there is a memory leak!!!!!
...
void Tree::variable_importance(vector<float>* score,
  ...
  for (uint i = 0; i < set_.num_attributes(); ++i) {
    if (vars_used_.find(i) != vars_used_.end()) {
      ...
      (*score)[i] = (float)(correct - permuted);
